...
class Bake:
    ...
    def __init__(self, highpoly_path, lod_path, resolution_tuple, to_bake_dict, export_directory): # I haven't given a choice of export name, because that adds so much complexity
        ...
        self.to_bake_dict = to_bake_dict
    ...
    def create_in_houdini(self, housing_node): # includes executing the baking
        ...
        for map_name in self.to_bake_dict.keys(): # it's a lot more clear doing this all here
            parameter_name = self.houdini_parameter_names[map_name]
            a_parm = baketexture_node.parm(parameter_name)

            bake_bool = self.to_bake_dict[map_name]

            # tick mark on is set 1 (and off is set 0)
            if bake_bool == True:
                a_parm.set(1) # set ticked
            else:
                a_parm.set(0) # set unticked

        hou.hipFile.save() # save hip file before executing
