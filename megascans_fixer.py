...
def main():
    ...
    #print(asset_geometry_node, asset_material_node, file_node, transform_node, rs_material_builder_node, redshift_material_node)

    print("2")
    megascans_asset_folder_path = os.path.dirname(file_node.parm("file").eval())
    #print(megascans_asset_folder_path)
    ...
    #map_list = get_maps(file_scan) # problematic if this doesn't get all the maps. No longer used
    ...
    if "Vector Displacement" in export_paths_dict_keys and "Displacement" in export_paths_dict_keys: # if both there, only set up Vector Displacement
        export_paths_dict.pop("Displacement") 

    if "Normal" in export_paths_dict_keys:
